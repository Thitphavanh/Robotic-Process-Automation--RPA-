...
class RPATaskAdmin(admin.ModelAdmin):
    list_display = ['name', 'task_type', 'status', 'keyword', 'created_at', 'updated_at']
...
class NewsSourceAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('การตั้งค่า Scraping', {
            'fields': ('selector', 'api_endpoint', 'api_key')
        }),
        ('วันที่และเวลา', {
...
class NewsArticleAdmin(admin.ModelAdmin):
    list_display = ['title', 'source', 'price', 'change_percent', 'sentiment', 'published_at']
    ...
    fieldsets = (
        ('ข้อมูลข่าว', {
            'fields': ('source', 'title', 'content', 'url', 'image_url')
        }),
        ('ข้อมูลราคา', {
            'fields': ('price', 'change', 'change_percent')
        }),
        ('AI Analysis', {
            'fields': ('ai_summary', 'sentiment')
        }),
        ('วันที่และเวลา', {
...
class DailyReportAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('ข้อมูลรายงาน', {
            'fields': ('report_date', 'is_completed', 'is_sent', 'sent_at')
        }),
        ('สรุปแต่ละหมวด', {
            'fields': (
                'stock_thai_summary',
